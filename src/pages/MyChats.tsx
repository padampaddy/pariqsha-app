        ...
        <div className="px-4 ">
          ...
          <ul>
            {data?.communication_threads.flatMap((chat, index) => (
              <Link role="button" to={`/messages/${chat.id}`} onClick={() => dispatch(modalSlice.actions.hideModal())}>
          ...
          <button className="fixed  text-white common-btn p-4 left-0 bottom-0 md:w-10/12  md:left-80 w-full">
            New Chat
          </button>
        </div>
      </div>
